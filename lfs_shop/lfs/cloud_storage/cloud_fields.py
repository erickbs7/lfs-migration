...
class CloudImage(object):
    ...
    def delete(self):
        if self.file_name is not None and self.upload_to is not None:
            # TODO: Refatorar - Adicionar deleção de thumbnails.
            r = cloud_storage.delete(self.upload_to + '/' + self.file_name)
            if r:
                print('Imagem deletada com sucesso.')
            else:
                print('Imagem falhou em ser deletada.')
            print('Nome da imagem: '+self.upload_to + '/' + self.file_name)
        else:
