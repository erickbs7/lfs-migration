...
from lfs.cloud_storage.cloud_fields import CloudImage
...
def delete_images(request):
    ...
    image_list = Image.objects.filter(pk__in=request.POST.getlist("images"))
...
def add_images(request):
    ...
    if request.method == "POST":
        for file_content in request.FILES.getlist("files[]"):
            cloud_image = CloudImage(upload_to='images', file_name='init.jpeg', sizes=((60, 60), (100, 100)))
            image = Image(title=file_content.name, image=cloud_image)
            try:
                # TODO: Refatorar - Adicionar tamanho de imagem 100x100
                # TODO: Refactoring point.
                image.image.save(file_name=file_content.name, content=file_content, upload_to='images')
                image.save()
            except Exception, e:
                print(e)
                image.delete()
