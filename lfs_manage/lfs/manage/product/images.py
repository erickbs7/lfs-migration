...
def add_image(request, product_id):
    ...
    if request.method == "POST":
        for file_content in request.FILES.getlist("files[]"):
            cloud_image = CloudImage(upload_to='images', file_name='test_file', sizes=THUMBNAIL_SIZES)
            image = Image(content=product, title=file_content.name, image=cloud_image)

            try:
                # TODO: Refactoring point.
                # image.image.save(file_content.name, file_content)
                image.image.save(content=file_content, file_name=file_content.name, upload_to='images')
                image.save()
